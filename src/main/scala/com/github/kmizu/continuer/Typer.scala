...
import com.github.kmizu.continuer.TypedAst.TypedNode
...
class Typer extends Processor[Ast.Program, TypedAst.Program] {
  ...
  def continuation(): TConstructor = {
    TConstructor("Continuation", List())
  }
  val BuiltinEnvironment: Environment = {
    Map(
      "continue"     -> TScheme(List(tv("a")), TFunction(List(continuation()), tv("a"))),
  ...
  def doType(e: Ast.Node, env: TypeEnvironment, t: Type, s0: Substitution): (TypedNode, Substitution) = {
    ...
    e match {
      ...
      case Ast.Capture(location, variable, body) =>
        if(env.variables.contains(variable)) {
          typeError(location, s"variable $variable is already defined")
        }

        val newEnv = env.updateImmuableVariable(variable, TScheme(Nil, continuation()))
        val (typedBody, s) = doType(body, newEnv, t, s0)
        (TypedAst.Capture(typedBody.type_, location, variable, continuation(), typedBody), s)
