...
import com.github.kmizu.continuer.Type.{TBoolean, TConstructor, TDynamic, TUnit}
  ...
  def doRewrite(node: Ast.Node): Ast.Node = node match {
    case Block(location, expressions) =>
      def rewriteBlock(es: List[Ast.Node]): List[Ast.Node] = es match {
        ...
        case SaveExpression(location, variable) :: xs =>
          List(Capture(location, variable, Block(location, rewriteBlock(xs))))
    ...
    case SaveExpression(location, variable) => SaveExpression(location, variable)
    case ContinueExpression(location, continuation) =>
      FunctionCall(
        location,
        Ast.Id(NoLocation, "continue"),
        List(continuation),
      )
