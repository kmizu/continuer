...
import com.github.kmizu.continuer.Ast._
...
class Parser extends Processor[String, Program] {
  object Core extends SCombinator {
    ...
    lazy val SAVE: Parser[String] = token("save")
    lazy val CONTINUE: Parser[String] = token("continue")
    ...
    lazy val KEYWORDS: Set[String] = Set(
      ...
      "def", "val", "mutable", "save", "continue", "=", "==", "=>", "new", "&", "|", "&&", "||"
    )
    ...
    //expression ::= assignment | ternary | ifExpression | whileEpression | saveExpression | continueExpression
    lazy val expression: Parser[Ast.Node] = rule(assignment | ternary | ifExpression | whileExpression | saveExpression | continueExpression)
    ...
    lazy val saveExpression: Parser[Ast.Node] = rule {
      (%% << CL(SAVE)) ~ commit(CL(ident)) ^^ {
        case location ~ variable => SaveExpression(location, variable.name)
      }
    }

    lazy val continueExpression: Parser[Ast.Node] = rule {
      (%% << CL(CONTINUE)) ~ commit(expression) ^^ {
        case location ~ continuation => ContinueExpression(location, continuation)
      }
    }
    ...
    lazy val unitLiteral: Parser[Ast.Node] = %% ~ """\(\)""".r ^^ {
      case location ~ _ => Id(location, "unit")
    }
    ...
    lazy val functionDefinition: Parser[FunctionDefinition] = rule {
      (%% << CL(DEF)) ~ commit(ident ~ (CL(LPAREN) >> (ident ~ typeAnnotation.?).repeat0By(CL(COMMA)) << CL(RPAREN)).? ~ (typeAnnotation.? << CL(EQ)) ~ expression) ^^ {
